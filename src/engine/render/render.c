...
SDL_Window *render_init(void)
{
	...
	render_init_shader(&shader_default, &shader_batch, render_width, render_height);
...
void render_quad_line(vec2 pos, vec2 size, vec4 color)
{
	...
	render_line_segment(point[0], point[1], color);
	render_line_segment(point[1], point[2], color);
	render_line_segment(point[2], point[3], color);
	render_line_segment(point[3], point[0], color);
}
...
void render_sprite_sheet_frame(Sprite_Sheet *sprite_sheet, f32 row, f32 column, vec2 position)
{
	...
	vec2 bottom_left = {position[0] - size[0] * 0.5,
