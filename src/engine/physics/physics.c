...
void aabb_penetration_vector(vec2 r, AABB aabb)
{
	...
	f32 min_dist = fabs(min[0]);
	r[0] = min[0];
	r[1] = 0;

	if (fabs(max[0]) < min_dist)
	{
		min_dist = fabs(max[0]);
		r[0] = max[0];
	}

	if (fabs(min[1]) < min_dist)
	{
		min_dist = fabs(min[1]);
		r[0] = 0;
		r[1] = min[1];
...
static void update_sweep_result(Hit *result, size_t other_id, AABB a, AABB b, vec2 velocity, u8 a_collision_mask, u8 b_collision_layer)
{
...
size_t physics_static_body_create(vec2 position, vec2 size, u8 collision_layer)
{
	...
	if (array_list_append(state.static_body_list, &static_body) == (size_t)-1)
		ERROR_EXIT("Could not append body to list \n");
