...
#define IO_READ_CHUNCK_SIZE 2097152
...
File io_file_read(const char *path)
{
	...
	if (!fp || ferror(fp))
		ERROR_RETURN(file, IO_READ_ERROR_GENERAL, path, strerror(errno));
	...
	while (true)
	{
		if (used + IO_READ_CHUNCK_SIZE + 1 > size)
		{

			size = used + IO_READ_CHUNCK_SIZE + 1;
		...
		n = fread(data + used, 1, IO_READ_CHUNCK_SIZE, fp);
		if (n == 0)
			break;
...
int io_file_write(void *buffer, size_t size, const char *path)
{
	...
	if (!fp || ferror(fp))
		ERROR_RETURN(1, IO_READ_ERROR_GENERAL, path, strerror(errno));
